...
class ContinentFavoriWidget(QWidget):
    ...
    def paintEvent(self, event) -> None:
        ...
        # --- badge (icône globe) ---
        rect_badge = QRectF(marge, h * 0.07, taille_badge, taille_badge)
        self._dessiner_badge_globe(painter, rect_badge)
    ...
    def _dessiner_badge_globe(self, painter: QPainter, rect: QRectF) -> None:
        """Badge circulaire en forme de globe (dégradé émeraude)."""
        ...
        contour = QPen(QColor(255, 255, 255, 225))
        contour.setWidthF(s * 0.035)
        painter.setPen(contour)
        painter.setBrush(Qt.BrushStyle.NoBrush)

        rayon_globe = s * 0.32
        rect_globe = QRectF(
            centre.x() - rayon_globe,
            centre.y() - rayon_globe,
            rayon_globe * 2,
            rayon_globe * 2,
        )
        painter.drawEllipse(rect_globe)

        # méridien (ellipse fine, plus étroite que le contour)
        rect_meridien = QRectF(
            centre.x() - rayon_globe * 0.42,
            centre.y() - rayon_globe,
            rayon_globe * 0.84,
            rayon_globe * 2,
        )
        ...
        # équateur (ligne horizontale)
        painter.drawLine(
            QPointF(centre.x() - rayon_globe, centre.y()),
            QPointF(centre.x() + rayon_globe, centre.y()),
        )

        # repère "continent favori" : petit point blanc en haut à droite du globe
        rayon_repere = s * 0.09
        point_repere = QPointF(
            centre.x() + rayon_globe * 0.55, centre.y() - rayon_globe * 0.6
        )
        painter.setPen(Qt.PenStyle.NoPen)
