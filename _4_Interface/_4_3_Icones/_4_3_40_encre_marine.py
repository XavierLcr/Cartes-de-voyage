...
# 4.3.40 – Script de création d'une encre marine                               #
################################################################################
...
image.fill(QColor("#EAF4F0"))

painter = QPainter(image)
painter.setRenderHint(QPainter.RenderHint.Antialiasing, True)

# petit fond de mer pour contexte : ciel, ligne de flottaison, mer
niveau_mer = H * 0.35

ciel = QLinearGradient(0, 0, 0, niveau_mer)
ciel.setColorAt(0.0, QColor("#BEE4F0"))
ciel.setColorAt(1.0, QColor("#DCF0EC"))
painter.fillRect(0, 0, W, int(niveau_mer), QBrush(ciel))

mer = QLinearGradient(0, niveau_mer, 0, H)
mer.setColorAt(0.0, QColor("#3E7C9B"))
mer.setColorAt(1.0, QColor("#173B54"))
painter.fillRect(0, int(niveau_mer), W, int(H - niveau_mer), QBrush(mer))

# trois bouées à différentes phases pour montrer le tangage / balancement de corde
positions = [
    (W * 0.25, math.pi * 0.0),
    (W * 0.50, math.pi * 0.9),
    (W * 0.75, math.pi * 1.6),
...
for x, phase in positions:
    centre = QPointF(x, niveau_mer)
    _dessiner_bouee(
        painter,
        centre,
        taille=140,
        profondeur_corde=H - niveau_mer - 20,
        couleur=QColor("#E85A2A"),
        phase=phase,
        degrade=True,
    )
